...
const INITIAL_LISTINGS = [
  // --- MAHSULLER (Her alt kategoriye tam 3 ilan) ---
  // Kiraz
  ...
  // --- CANLI HAYVANLAR (Her alt kategoriye tam 3 ilan) ---
  // Büyükbaş
  ...
  // --- HAYVAN YEMLERİ VE EKİPMANLARI (Her alt kategoriye tam 3 ilan) ---
  // Yem Çeşitleri
  ...
  // --- ARICILIK & BAL (Her alt kategoriye tam 3 ilan) ---
  // Süzme Bal
  ...
  // --- ARICILIK EKİPMANLARI (Her alt kategoriye tam 3 ilan) ---
  // Kovan
  ...
  // --- TRAKTÖR (Her alt kategoriye tam 3 ilan) ---
  // İkinci El Traktör
  ...
  // --- TARIM EKİPMANLARI (Her alt kategoriye tam 3 ilan) ---
  // Römork
  ...
  // --- TARIM İŞÇİLERİ (Her alt kategoriye tam 3 ilan) ---
  // Hasat Ekibi
...
export default function App() {
  ...
  useEffect(() => {
    // Tarayıcı önbelleğini sıfırlamak için yeni versiyon anahtarı kullanıyoruz
    localStorage.setItem('pazartarla_listings_v3', JSON.stringify(INITIAL_LISTINGS));
    setListings(INITIAL_LISTINGS);
  }, []);

  const saveListings = (newListings) => {
    setListings(newListings);
    localStorage.setItem('pazartarla_listings_v3', JSON.stringify(newListings));
  };
